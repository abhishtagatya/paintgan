...
class CheckpointMonitor(tf.keras.callbacks.Callback):
    ...
    def __init__(self, directory, checkpoint_per=10):
        self.directory = directory
        self.model_name = 'AdaIN'
        self.checkpoint_per = checkpoint_per

    def on_epoch_end(self, epoch, logs=None):
        # Saving model checkpoint
        if (epoch + 1) % self.checkpoint_per == 0:
            self.model.save_weights(f'{self.directory}/{self.model_name}_{epoch + 1}.ckpt')
...
class DisplayMonitor(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        plt.savefig(f'{self.directory}/{self.model_name}_{epoch + 1}.png', format='png')
